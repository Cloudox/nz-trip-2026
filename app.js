...
(function () {
  ...
  function segFlight() {
    ...
    return `<div class="sec-title">航班信息（确认函版）</div>
      ...
      <div class="qnote" style="text-align:center;margin:2px 0 10px">数据版本 ${esc(APP_VERSION)} · 看不到航班分段就下拉刷新一次</div>`;
  }
  ...
  if ("serviceWorker" in navigator && location.protocol.indexOf("http") === 0) {
    let allowReload = false, didReload = false;
    navigator.serviceWorker.addEventListener("controllerchange", () => {
      // 只在"发现新版本"时刷新，避免首次安装也刷新
      if (allowReload && !didReload) { didReload = true; location.reload(); }
    });
